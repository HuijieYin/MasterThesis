...
def get_data(args: Any) -> Dict[str, Union[np.ndarray, pd.DataFrame, None]]:
    ...
    """
    ########### Delete baseline_key ###############
    input_features = BASELINE_INPUT_FEATURES
    output_features = BASELINE_OUTPUT_FEATURES
    ##########################
...
def validate_args(args: Any) -> bool:
    ...
    if args.use_social and args.use_map:
        ...
        success = False
    if args.obs_len > 50:
        print("[ARGS ERROR]: obs_len cannot be more than 50.")
        success = False
    if args.pred_len > 60:
        print("[ARGS ERROR]: pred_len cannot be more than 60.")
        success = False
    return success