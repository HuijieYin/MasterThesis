...
from utils.baselineConfig import (
    PADDING_TYPE,
    RAW_DATA_FORMAT,
...
# https://github.com/jagjeet-singh/argoverse-forecasting/tree/master/utils
...
class SocialFeaturesUtils:
    ...
    # todo: remove because av2 has velocity.
    def compute_velocity(self, track_df: pd.DataFrame) -> List[float]:
        ...
        """
        vel_x, vel_y = track_df["velocity_x"].values, track_df["velocity_y"].values
    ...
    def pad_track(
        ...
        track_timestamps = track_df["timestep"].values
        ...
        for i in range(padded_track_array.shape[0]):
            padded_track_array[i, self.RAW_DATA_FORMAT["timestep"]] = seq_timestamps[i]
    ...
    def filter_tracks(self, seq_df: pd.DataFrame, obs_len: int,
        ...
        Args:
            seq_df (pandas Dataframe): Dataframe containing all the tracks in the sequence except focal track
            obs_len (int): Length of observed trajectory
            raw_data_format (Dict): Format of the sequence
        Returns:
        ...
        seq_timestamps = np.unique(seq_df["timestep"].values)
    ...
    def get_is_front_or_back(
        ...
        """
        # todo: seems like we have heading information then we need modify this part.
...
# 对读取进来的数据进行处理。将focal_track和social_track提取出来并获得groudtruth

    def compute_social_features(
            self,
            df: pd.DataFrame, #AV2的训练数据返回的是表格dataframe
            focal_track: np.ndarray,
            obs_len: int,
            seq_len: int,
            raw_data_format: Dict[str, int],
        ...
        Args:
            ...
            focal_track (numpy array): Data for the agent track
            obs_len (int): Length of observed trajectory
        ...
        """
        agent_ts = np.sort(np.unique(df["timestep"].values))

        if agent_ts.shape[0] == obs_len:
            df_obs = df
            focal_track_obs = focal_track.reshape((1, obs_len, -1))
            focal_track_gt = None
        else:
            # Get obs dataframe and agent track
            df_obs = df[df["timestep"] < agent_ts[obs_len]]
            assert (np.unique(df_obs["timestep"].values).shape[0] == obs_len
                    ), "Obs len mismatch"
            focal_track_obs = focal_track[:obs_len].reshape((1, obs_len, -1))
            focal_track_gt = focal_track[obs_len:].reshape((1, seq_len-obs_len, -1))
        ...
        return focal_track_obs, social_tracks_obs, focal_track_gt
